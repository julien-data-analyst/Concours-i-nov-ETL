...
def insert_bdd(cur, conn):
    """
    Insert the data cleaned from two datasource (concours.jsonl and concours_projet_thematique.jsonl)
    
    :param cur: connection instance for executing SQL command
    :param conn: connection instance to commit the different insert
    """
    ###################################################-
    ...
    if result == []:
        ...
        projets_them = pd.merge(projets, id_them, how="left", on="THEMATIQUE")
        projets_comp = pd.merge(projets_them, id_comp, how='left', on="ENTREPRISE")

        projets_fin = projets_comp[["PROJET", "PAGE", "OBJECTIF_PROJET", "MONTANT_PROJET_EXTRAITE", "MONTANT_AIDE_EXTRAITE", 
        ...
        for projet in projets_fin.values:
    ...
    if result == []:
        # Récupérer les numéros de départements
        cur.execute("SELECT id, dep_number FROM Department;")
        id_dep = pd.DataFrame(cur.fetchall(), columns=["id_dep", "NUMERO_DEPARTEMENT"])

        cur.execute("SELECT id, name FROM Project;")
        id_proj = pd.DataFrame(cur.fetchall(), columns=["id_proj", "PROJET"])

        # Garder nom du projet et les numéros de départements
        projets_dep = pd.merge(projets[["PROJET", "NUMERO_DEPARTEMENT"]][projets["NUMERO_DEPARTEMENT"].apply(len) > 0], id_proj, how="left", on="PROJET")

        #print(projets_dep)

        # Chercher les lignes à insérer
        liste_location = []

        for proj in projets_dep.values:
            for dep in proj[1]:
                if dep != "00" and dep != "20":
                    #print(dep)
                    val = id_dep[id_dep["NUMERO_DEPARTEMENT"] == dep].iloc[0]
                    #print(val["id_dep"])
                    
                    liste_location.append([int(val["id_dep"]), proj[2]])

        #print(liste_location)

        # Insérer les lignes trouvées
        for lign in liste_location:
            cur.execute("INSERT INTO Location (id_dep, id_proj) VALUES (%s, %s);", lign)
...
if __name__=="__main__":
    # Importer les libairies nécessaires à l'exécution de la fonction
    import sys
